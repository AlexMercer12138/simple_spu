`timescale 1ns / 1ps
//================================================================================
//  Module      : tinyc_uart_irq_tb
//  Description : Tiny C firmware interrupt integration test for MERC32 and UART
//================================================================================
...
module tinyc_uart_irq_tb();
    ...
    wire ready_write = dlb_en && dlb_we &&
                       (dlb_addr == STATUS_ADDR) &&
                       (dlb_wdata == READY_CODE);
    ...
    task receive_uart_byte;
        ...
        begin
            ...
            if (uart_tx !== 1'b1) begin
                $display("[FAIL] UART stop bit is not high for byte %0d",
                         received_count);
    ...
    initial begin
        wait (rst_n);
        wait (ready_seen);
        for (received_count = 0; received_count < 2;
            received_count = received_count + 1) begin
            send_uart_byte(expected_uart_byte(received_count));
            while (dlb_ram[IRQ_COUNT_ADDR] != (received_count + 1))
                @(posedge clk);
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (firmware_fail_write) begin
                ...
                $display("TEST FAIL: firmware status=0x%08h detail=0x%08h irq_count=0x%08h UART errors=%0d",
                         dlb_wdata, dlb_ram[FAIL_ADDR],
                         dlb_ram[IRQ_COUNT_ADDR], uart_error_count);
                $finish;
            end else if ((firmware_pass_seen || firmware_pass_write) &&
                         uart_sequence_done) begin
                done <= 1'b1;
                if ((dlb_ram[IRQ_COUNT_ADDR] == 32'd2) &&
                    (uart_error_count == 0))
                    $display("TEST PASS");
                else
                    $display("TEST FAIL: firmware passed with irq_count=0x%08h UART errors=%0d",
                             dlb_ram[IRQ_COUNT_ADDR], uart_error_count);
                $finish;
            end else if (cycle_count >= MAX_CYCLES) begin
                done <= 1'b1;
                $display("TEST TIMEOUT: pc=%0d status=0x%08h detail=0x%08h irq_count=0x%08h ready=%0d sequence_done=%0d uart_bytes=%0d UART errors=%0d",
                         MERC32_top_inst.u_merc32_core.prog_addr,
                         dlb_ram[STATUS_ADDR], dlb_ram[FAIL_ADDR],
                         dlb_ram[IRQ_COUNT_ADDR], ready_seen,
                         uart_sequence_done, received_count,
                         uart_error_count);
                $finish;
            end
        end
    ...
    initial #(CLK_PERIOD*(MAX_CYCLES + RESET_CYCLES + 1000)) begin
        if (!done) begin
            $display("TEST TIMEOUT: testbench watchdog status=0x%08h irq_count=0x%08h ready=%0d sequence_done=%0d UART errors=%0d",
                     dlb_ram[STATUS_ADDR], dlb_ram[IRQ_COUNT_ADDR],
                     ready_seen, uart_sequence_done, uart_error_count);
            $finish;
        end
    end

    // initial begin
    //     $dumpfile("tinyc_uart_irq_tb.vcd");
    //     $dumpvars(0, tinyc_uart_irq_tb);
    // end
