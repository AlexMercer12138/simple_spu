...
assert.strictEqual(normalizeSectionContent({ name: 'text', alignment: 4, size: 4, content: 'main: jmp external, r14' }).length, 1);
assert.doesNotThrow(() => validateObject(assembleToObject('main: jmp external, r14\n')));
...
assert.doesNotThrow(() => validateObject({ ...object, symbols: [...object.symbols, { name: 'main', binding: 'local', section: 'text', offset: 0, defined: true }] }));
assert.doesNotThrow(() => validateObject({ ...object, symbols: [...object.symbols, { name: 'local', binding: 'local', section: 'text', offset: 0, defined: true }] }));
