...
字符串按 UTF-8 保存，转义规则与字符字面量相同；相邻字符串先拼接，再追加一个 NUL 字节。字符串表达式的类型为 `char *`，可用于全局或局部指针初始化，也可直接作为函数实参：
...
编译器在 DLB 中所有用户全局对象之后分配隐藏静态字符串，存放 UTF-8 payload 和结尾 NUL，并按完整字节内容去重。启动代码使用 `sb` 写入字符串。该内存由硬件视为普通可写 RAM；字符串字面量只按语言约定只读，编译器和硬件都不提供写保护。需要修改文本时应使用字符数组。

### 数组初始化

一维 `char`、`unsigned char`、`short`、`unsigned short`、`int` 和 `unsigned int` 数组支持固定长度或从非空初始化内容推导长度：
...
列表允许尾逗号；固定长度多出的元素补零，初始化项过多则报错。全局数组列表只能使用整数常量表达式，由启动代码按元素宽度以 `sb`、`sh` 或 `sw` 写入；局部数组元素可使用运行时表达式，在声明处按源码顺序各求值一次，随后补零。

字符串初始化仅适用于 `char` / `unsigned char` 数组，直接复制 UTF-8 payload 和 NUL，不额外使用字符串池副本。省略长度时推导为 UTF-8 payload 字节数加 1；固定容量必须容纳 payload 和 NUL，剩余元素补零。
