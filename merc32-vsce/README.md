...
`0x10000000`。传统 Tiny C 后端的直接标签跳转使用汇编器的有符号 16 位
...
上述是语言子集限制。传统 Tiny C 后端当前生成直接标签跳转/分支，汇编器再把标签
替换为有符号 16 位绝对立即数；编译器会拒绝超出 `0x00007FFF` 的 `codeBase`，
并在非零 `codeBase` 下检查生成代码是否仍落在该范围内。默认零基址的大程序若
标签地址超界，仍会由汇编器报告；目前没有自动的 far-label 跳板或长跳转展开。
