import { CompilerError } from './tinyc';

export type TokenKind = 'identifier' | 'keyword' | 'number' | 'string' | 'char' | 'symbol' | 'eof';
export interface Token { readonly kind: TokenKind; readonly text: string; readonly value?: number; readonly line: number; readonly column: number; }
...
export function tokenizeC(source: string): Token[] {
  ...
  const token = (kind: TokenKind, text: string, l: number, c: number, value?: number): Token => ({ kind, text, line: l, column: c, ...(value === undefined ? {} : { value }) });
  while (i < source.length) {
    ...
    throw new CompilerError(`unexpected character '${ch}'`, l, c);
  }
