...
class FunctionLowerer {
    ...
    private lowerLValueAddress(expression: LoweringExpression): number { if (expression.kind === 'declaration-reference' || expression.kind === 'identifier') { const binding = expression.binding ?? expression.symbol; return binding && this.variables.has(binding) ? this.addressLocal(binding) : this.addressSymbol(expression.symbol ?? '', expression.location); } if (expression.kind === 'unary' && expression.operator === 'parentheses') return this.lowerLValueAddress(expression.operands[0]); if (expression.kind === 'unary' && expression.operator === '*') return this.lowerExpression(expression.operands[0]); if (expression.kind === 'subscript') { const element = expression.type; return this.addAddressOffset(this.lowerExpression(expression.operands[0]), this.scalePointerOffset(this.lowerExpression(expression.operands[1]), typeSize(element), expression.location), expression.location); } if (expression.kind === 'member') { const baseType = unwrapType(expression.operands[0].type); const base = baseType.kind === 'pointer' ? this.lowerExpression(expression.operands[0]) : this.lowerLValueAddress(expression.operands[0]); return expression.memberOffset ? this.addAddressOffset(base, this.constant(expression.memberOffset, expression.location), expression.location) : base; } throw new Error(`typed lowering cannot take the address of '${expression.kind}'`); }
