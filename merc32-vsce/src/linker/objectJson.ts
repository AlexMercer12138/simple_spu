...
export function validateObject(object: Merc32Object): void {
  ...
  for (const section of object.sections) {
    ...
    if (!Number.isInteger(section.size) || section.size < 0) throw new Error('invalid section size');
    if (!Number.isInteger(section.alignment) || section.alignment <= 0 || (section.alignment & (section.alignment - 1)) !== 0) throw new Error('invalid section alignment');
