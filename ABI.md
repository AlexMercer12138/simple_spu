...
`__irq_handler` 仍按普通 Tiny C 函数生成栈帧和函数体，区别仅在 epilogue 恢复普通函数帧后执行 `jmp r3`，而不是 `jmp r14`。编译器不会额外保存被中断代码的 `r4-r11` 或其他上下文，也不会隐式重新使能中断或清除外设中断源。

需要保留完整上下文的程序必须自行保存和恢复 `r1-r15` 中会被破坏的寄存器。比较结果已经是普通寄存器中的 `0/1`，需要跨中断保留时也按普通寄存器处理。软件应自行决定是否在返回前调用 `__irq_enable()`、是否允许嵌套，以及何时清除中断源；`jmp r3` 只是普通间接跳转，不会触发额外的硬件恢复动作。重复触发、嵌套覆盖 `r3` 或未清源导致的程序跑飞均由裸机软件负责。
